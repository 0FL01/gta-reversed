#!/usr/bin/python3
"""Build/run the isolated sprite33 HUD EGL probe with Ninja-derived flags.

Run inside mad-sa-graphics-build. Outputs only artifacts/graphics and reads
the original /game mount without copying assets.
"""
import pathlib
import shlex
import subprocess
...
names = ('MenuShot', 'RadarMap', 'NativeScriptEntities', 'RealtimeHudSaveProbe')
objects = []
logfile = output / 'RealtimeHudSaveProbe-build.log'
with logfile.open('w') as log:
    for name in names:
        ...
        while i < len(template):
            ...
            if arg in ('-MT', '-MF', '-o', '-c'):
                ...
                command.append(arg)
                i += 1
        obj = output / ('RealtimeHudSave-' + name + '.o')
    ...
    link[link.index('-o') + 1] = str(output / 'RealtimeHudSaveProbe')
    link[1:1] = objects
    ...
    if result.returncode:
        ...
        print(logfile.read_text())
        result.check_returncode()
with (output / 'RealtimeHudSaveProbe.log').open('w') as log:
    result = subprocess.run(
        [str(output / 'RealtimeHudSaveProbe'), '/game'], cwd=workspace,
        stdout=log, stderr=subprocess.STDOUT
    )
print((output / 'RealtimeHudSaveProbe.log').read_text(), end='')
result.check_returncode()
