...
bool GameShot_BlitRadarDisc(std::vector<uint8_t>& dst, const std::vector<uint8_t>& radarFull,
    ...
    copiedOut = 0;
    if (dst.size() != static_cast<std::size_t>(kFbW) * kFbH * 4 ||
        radarFull.size() != static_cast<std::size_t>(kFbW) * kFbH * 4) {
        SetErr(err, errSize, "radar blit frame has bad size");
        return false;
    }
    if (discR <= 0 || discR > kFbW) {
        SetErr(err, errSize, "radar blit bad discR");
        return false;
    }
    long copied = 0;
    const int r2 = discR * discR;
    for (int dy = -discR; dy <= discR; ++dy) {
        for (int dx = -discR; dx <= discR; ++dx) {
            if (dx * dx + dy * dy > r2) {
                continue;
            }
            ...
            uint8_t* d =
                dst.data() + (static_cast<std::size_t>(dby) * kFbW + dxp) * 4;
            d[0] = src[0];
            d[1] = src[1];
            d[2] = src[2];
            d[3] = 255;
...
bool GameShot_RenderWeatherHour(const char* gameDir, int health, int armor, const char* weather,
                                 ...
                                 std::vector<uint8_t>& gamePixels, GameShotStats& stats, char* err,
                                 std::size_t errSize) {
    return GameShot_RenderWeatherHourFog(gameDir, health, armor, weather, hour, false, basePixels,
                                         hudPixels, radarFull, gamePixels, stats, err, errSize);
}
...
bool GameShot_RenderWeatherHourFog(const char* gameDir, int health, int armor, const char* weather,
    ...
    // R6an: wantFog renders the base through the scene --fog rasterizer
    // path (HUD/radar overlays below stay unfogged: base-only order).
    {
        char herr[768] = {};
        if (!HudShot_RenderWeatherHourFog(gameDir, health, armor, weather ? weather : "EXTRASUNNY_LA",
                                          hour, wantFog, basePixels, hudPixels, stats.hud, herr,
                                          sizeof(herr))) {
            char msg[896];
    ...
    // R6as: shared helper (also used by the drive-hud --radar path).
    ...
    {
        char berr[256] = {};
        if (!GameShot_BlitRadarDisc(gamePixels, radarFull, r, copied, berr, sizeof(berr))) {
            char msg[384];
            (void)std::snprintf(msg, sizeof(msg), "radar-blit: %s", berr);
            SetErr(err, errSize, msg);
            GameShot_Shutdown();
            return false;
        }
