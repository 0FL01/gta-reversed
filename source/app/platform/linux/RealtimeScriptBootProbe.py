...
if __name__ == '__main__':
    ...
    if args.build:
        ...
        assert len(re.findall(r'play-script-terminal status=Unsupported thread=1 generation=1 ip=200868 opcode=0517 executed=117 ', result.stdout)) == 1
        assert 'play-ok' not in result.stdout and 'play-fail' not in result.stdout and 'FAIL' not in result.stdout
        print('boot-probe PASS actual-startup-frame=1 mission-prefix=117 terminal=0517@200868 runtime-exit=1 fullboot=0')
