#!/usr/bin/env python3
"""Build/run the isolated owned save-pickup collection probe."""
import argparse
import pathlib
import shlex
import subprocess
...
NAME = 'NativePickupCollectionProbe'
...
def build():
    ...
    units = ('NativePlayerActivity', 'NativeScriptEntities', NAME)
    removed = units + ('MainLinux', 'Realtime')
    link = [arg for arg in link if not any(arg.endswith('/' + unit + '.cpp.o') for unit in removed)]
    # A mounted persistent Conan cache can retain the same package with a new
    # recipe hash after configure. Keep Ninja's link shape, resolving only a
    # missing explicit package-library path by its unique cached basename.
    ...
    with (OUTPUT / (NAME + '-build.log')).open('w') as log:
        for unit in units:
            obj = OUTPUT / ('pickup-collection-' + unit + '.o')
