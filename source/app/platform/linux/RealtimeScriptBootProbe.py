...
if __name__ == '__main__':
    ...
    if args.build:
        ...
        assert len(terminals) == 1 and terminals[0].startswith('play-script-terminal status=Unsupported thread=1 generation=1 ip=207007 opcode=014B executed=167 '), terminals
        assert 'play-ok' not in result.stdout and 'play-fail' not in result.stdout and 'FAIL' not in result.stdout
        print('boot-probe PASS actual-captures=3 mission-quanta=0/256/512+167 mission-prefix=679 terminal=014B@207007 '
