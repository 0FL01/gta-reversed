#!/usr/bin/env python3
"""Build/run the isolated source-backed native car-generator probe."""
import argparse
import pathlib
import shlex
import subprocess
...
def build():
    ...
    with (OUTPUT / (NAME + '-build.log')).open('w') as log:
        for unit in ['NativeVehiclePool', 'NativeCarGenerators', NAME]:
            obj = OUTPUT / ('cargen-' + unit + '.o')
