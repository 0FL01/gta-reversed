...
// R6am (round 41): optional --weather W (only with --hour, like
// --shot-scene) renders the shore base from section W row H (same
...
int RunShotGame(int argc, char** argv) {
    ...
    // R6am (round 41): optional weather section. Only with --hour (like
    // --shot-scene: bare --weather fails honestly); the token must be a
    // valid section name (A-Z0-9_), matched exactly in TimeCycle (so
    // SUNNY_LA never hits EXTRASUNNY_LA). Without the flag the legacy
    // EXTRASUNNY_LA path stays bit-for-bit.
    char weatherName[64] = {};
    ...
    bool hasWeather = false;
    {
        ...
        if (weatherArg) {
            ...
            if (!wok) {
                (void)std::printf("game-fail bad --weather '%s' (want SECTION like CLOUDY_LA)\n",
                                   weatherArg);
                return 1;
            }
    ...
    (void)std::printf("shore-cam eye=%.1f,%.1f,%.1f target=%.1f,%.1f,%.1f fov=60\n",
                       sst.eye[0], sst.eye[1], sst.eye[2], sst.target[0], sst.target[1],
                       sst.target[2]);
    // R6al: with --hour the same timecyc row drives the base (water via
    ...
    // R6am: with --weather the row comes from the named section instead
    // (same exact-token path as --shot-scene --weather W --hour H).
    if (hasHour) {
        TimeCycleParams tcp{};
        char tcErr[256] = {};
        if (!TimeCycle_LoadWeatherHour(gameDir.c_str(), weather, hour, tcp, tcErr,
                                       sizeof(tcErr))) {
            (void)std::printf("game-fail timecyc %s (game=%s weather=%s hour=%d)\n", tcErr,
        ...
        (void)std::printf(
            ...
            "skytop=%d,%d,%d skybot=%d,%d,%d suncore=%d,%d,%d sample=%s sunDir=fixed spec=off\n",
            weather, hour, tcp.amb[0], tcp.amb[1], tcp.amb[2], tcp.dir[0], tcp.dir[1],
    ...
    if (hasHour) {
        if (showZone && wanted >= 0 && hasMoney) {
            (void)std::printf(
                ...
                "hudChecksum=%llu radarChecksum=%llu zoneLabel=1 wanted=%d money=%d %shour=%d "
                "checksum=%llu\n",
            ...
            (void)std::printf(
                "game-ok health=%d armor=%d hudPixels=%ld radarPixels=%ld baseChecksum=%llu "
                "hudChecksum=%llu radarChecksum=%llu zoneLabel=1 wanted=%d %shour=%d checksum=%llu\n",
                gst.hud.health, gst.hud.armor, gst.hud.hudPixels, gst.radarPixels,
                static_cast<unsigned long long>(baseChecksum),
                static_cast<unsigned long long>(hudChecksum),
            ...
            (void)std::printf(
                "game-ok health=%d armor=%d hudPixels=%ld radarPixels=%ld baseChecksum=%llu "
                "hudChecksum=%llu radarChecksum=%llu zoneLabel=1 money=%d %shour=%d checksum=%llu\n",
                gst.hud.health, gst.hud.armor, gst.hud.hudPixels, gst.radarPixels,
                static_cast<unsigned long long>(baseChecksum),
                static_cast<unsigned long long>(hudChecksum),
            ...
            (void)std::printf(
                "game-ok health=%d armor=%d hudPixels=%ld radarPixels=%ld baseChecksum=%llu "
                "hudChecksum=%llu radarChecksum=%llu wanted=%d money=%d %shour=%d checksum=%llu\n",
                gst.hud.health, gst.hud.armor, gst.hud.hudPixels, gst.radarPixels,
                static_cast<unsigned long long>(baseChecksum),
                static_cast<unsigned long long>(hudChecksum),
                static_cast<unsigned long long>(radarChecksum), wanted, money, hourPrefix, hour,
            ...
            (void)std::printf(
                "game-ok health=%d armor=%d hudPixels=%ld radarPixels=%ld baseChecksum=%llu "
                "hudChecksum=%llu radarChecksum=%llu money=%d %shour=%d checksum=%llu\n",
                gst.hud.health, gst.hud.armor, gst.hud.hudPixels, gst.radarPixels,
                static_cast<unsigned long long>(baseChecksum),
                static_cast<unsigned long long>(hudChecksum),
                static_cast<unsigned long long>(radarChecksum), money, hourPrefix, hour,
            ...
            (void)std::printf(
                "game-ok health=%d armor=%d hudPixels=%ld radarPixels=%ld baseChecksum=%llu "
                "hudChecksum=%llu radarChecksum=%llu zoneLabel=1 %shour=%d checksum=%llu\n",
                gst.hud.health, gst.hud.armor, gst.hud.hudPixels, gst.radarPixels,
                static_cast<unsigned long long>(baseChecksum),
                static_cast<unsigned long long>(hudChecksum),
            ...
            (void)std::printf(
                "game-ok health=%d armor=%d hudPixels=%ld radarPixels=%ld baseChecksum=%llu "
                "hudChecksum=%llu radarChecksum=%llu wanted=%d %shour=%d checksum=%llu\n",
                gst.hud.health, gst.hud.armor, gst.hud.hudPixels, gst.radarPixels,
                static_cast<unsigned long long>(baseChecksum),
                static_cast<unsigned long long>(hudChecksum),
                static_cast<unsigned long long>(radarChecksum), wanted, hourPrefix, hour,
            ...
            (void)std::printf(
                "game-ok health=%d armor=%d hudPixels=%ld radarPixels=%ld baseChecksum=%llu "
                "hudChecksum=%llu radarChecksum=%llu %shour=%d checksum=%llu\n",
                gst.hud.health, gst.hud.armor, gst.hud.hudPixels, gst.radarPixels,
                static_cast<unsigned long long>(baseChecksum),
                static_cast<unsigned long long>(hudChecksum),
                static_cast<unsigned long long>(radarChecksum), hourPrefix, hour,
...
#define EGL_PLATFORM_SURFACELESS_MESA 0x31DD
    ...
    // R6as (round 47): optional --radar overlays a RadarMap disc centered on
    // the car XY of every waypoint (existing RadarMap path + existing
    ...
    if (wantHud) {
        ...
        if (useHandling) {
            hudFromHandlingPath = true;
            hudVmaxMs = hp.vmaxMs;
            for (int i = 0; i < waypoints; ++i) {
                hudVel.push_back(wps[static_cast<size_t>(i)].vel);
            }
        } else {
            // Display-only side channel: handling row + DriveSim integrator
            // give the overlay speeds; geometry/render stay kinematic.
            HandlingParams hudHp{};
            char hhErr[512] = {};
            if (!Handling_Load(gameDir.c_str(), model, hudHp, hhErr, sizeof(hhErr))) {
                (void)std::printf("drivehud-fail handling %s (game=%s model=%s)\n", hhErr,
                                   gameDir.c_str(), model);
                DriveSim_ShutdownWorld();
                eglMakeCurrent(display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
                eglDestroyContext(display, context);
            ...
            (void)std::printf("drivehud-handling model=%s mass=%s vmax=%s accel=%s drag=%s "
                               "gears=%s displayOnly=1\n",
                               hudHp.requested, hudHp.massTok, hudHp.vmaxTok, hudHp.accelTok,
                               hudHp.dragTok, hudHp.gearsTok);
            std::vector<DriveWaypoint> hudWps;
            std::vector<DriveSimTrace> hudTrace;
            double hudT = 0.0;
            char hsErr[512] = {};
            if (!DriveSim_SampleHandling(ctrl, waypoints, meas.wheelbase, meas.wheelR,
                                         hudHp.vmaxMs, hudHp.accelSi, 1.0 / 30.0, hudWps,
                                         hudTrace, hudT, hsErr, sizeof(hsErr))) {
                (void)std::printf("drivehud-fail hud-sim %s\n", hsErr);
                DriveSim_ShutdownWorld();
                eglMakeCurrent(display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
                eglDestroyContext(display, context);
                eglDestroySurface(display, surface);
            ...
            hudVmaxMs = hudHp.vmaxMs;
            for (int i = 0; i < waypoints; ++i) {
                hudVel.push_back(hudWps[static_cast<size_t>(i)].vel);
            }
        }
        hudSpdTexts.reserve(static_cast<size_t>(waypoints));
        hudKmh.reserve(static_cast<size_t>(waypoints));
        for (int i = 0; i < waypoints; ++i) {
            double v = hudVel[static_cast<size_t>(i)];
            int kmh = static_cast<int>(std::lround(v * 3.6));
            if (kmh < 0) {
                kmh = 0;
            }
            if (kmh > 999) {
        ...
        bool hudMono = true;
        for (int i = 1; i < waypoints; ++i) {
            if (!(hudVel[static_cast<size_t>(i)] >=
                  hudVel[static_cast<size_t>(i - 1)] - 1e-9)) {
                hudMono = false;
            }
        }
        bool hudClamped = false;
        for (int i = 0; i < waypoints; ++i) {
            if (hudVel[static_cast<size_t>(i)] >= hudVmaxMs - 1e-6) {
                hudClamped = true;
            }
        }
        ...
        for (int i = 0; i < waypoints; ++i) {
            (void)std::printf("drivehud-wp i=%d v=%.2f kmh=%d text=\"%s\"\n", i,
                               hudVel[static_cast<size_t>(i)], hudKmh[static_cast<size_t>(i)],
                               hudSpdTexts[static_cast<size_t>(i)].c_str());
        }
        (void)std::printf("drivehud-speed mono=%d clamped=%d vmaxMs=%.6f note=%s\n",
                           hudMono ? 1 : 0, hudClamped ? 1 : 0, hudVmaxMs,
                           hudClamped ? "cruise-constant-at-VMAX(last-legs-equal)"
                                      : "accelerating(mono-ramp)");
        if (!hudMono) {
            (void)std::printf("drivehud-fail non-monotonic speeds\n");
            DriveSim_ShutdownWorld();
            eglMakeCurrent(display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
            eglDestroyContext(display, context);
            eglDestroySurface(display, surface);
    ...
    for (int i = 0; i < waypoints && !failed; ++i) {
        ...
        if (!wantHud) {
            ...
            // R6as: radar disc centered on THIS waypoint car XY through the
            // EXISTING RadarMap path (gta3.img tiles only) + the EXISTING
            // GameShot 1:1 disc blit (radarPos=87,409, same composite path as
            // the game frame). Fixed-pier disc would log centerTex=radar115
            // on every frame; here centerTex follows w.x/w.y (see log).
            if (wantRadar) {
                std::vector<uint8> radarFull;
                RadarMapStats rst{};
                char rerr[768] = {};
                if (!RadarMap_Render(gameDir.c_str(), w.x, w.y, radarFull, rst, rerr,
                                     sizeof(rerr))) {
                    (void)std::printf("drivehud-fail radar %s (wp=%d x=%.2f y=%.2f)\n",
                                       rerr, i, w.x, w.y);
                    failed = true;
                    break;
                }
                long copied = 0;
                char berr[256] = {};
                if (!GameShot_BlitRadarDisc(hudPixels, radarFull, rst.discR, copied, berr,
                                            sizeof(berr))) {
                    (void)std::printf("drivehud-fail radar-blit %s (wp=%d)\n", berr, i);
                    failed = true;
                    break;
                }
                if (copied <= 20000) {
                    (void)std::printf(
                        "drivehud-fail thin radar wp=%d radarPixels=%ld (want >20000)\n", i,
                        copied);
                    failed = true;
                    break;
                }
                radarPer.push_back(copied);
                radarTex.emplace_back(rst.firstTile);
            ...
            uint64_t total = static_cast<uint64_t>(width) * static_cast<uint64_t>(height);
            if (nonBlack == 0) {
                (void)std::printf("drivehud-fail black frame wp=%d\n", i);
                failed = true;
                break;
            }
            if (hudChanged <= 2000) {
                (void)std::printf("drivehud-fail thin overlay wp=%d hudPixels=%ld(need >2000)\n",
                                   i, hudChanged);
                failed = true;
                break;
            }
            ...
            if (!WriteTga24(outPath, width, height, hudPixels)) {
                (void)std::printf("drivehud-fail write '%s'\n", outPath);
                failed = true;
                break;
            }
            checksums.push_back(checksum);
            hudPer.push_back(hudChanged);
            (void)std::printf("drivehud-shot wp=%d out=%s carTris=%d worldTris=%d "
    ...
    // R6ao: HUD-trip final. Hp is the SUM over waypoints (per-frame counts
    // are in the drivehud-shot lines above); S is the comma-joined SPD texts.
    long hudSum = 0;
    for (long v : hudPer) {
        hudSum += v;
    }
    ...
    // R6as: radar summary (only with --radar; without it this block never
    // runs and all legacy drivehud-ok lines below stay bit-identical).
    long radarSum = 0;
    if (wantRadar) {
        if (static_cast<int>(radarPer.size()) != waypoints ||
            static_cast<int>(radarTex.size()) != waypoints) {
            (void)std::printf("drivehud-fail radar count %d/%d (want %d)\n",
                               static_cast<int>(radarPer.size()),
                               static_cast<int>(radarTex.size()), waypoints);
            DriveSim_ShutdownWorld();
            return 1;
        }
        ...
        if (!(radarSum > 20000)) {
            (void)std::printf("drivehud-fail thin radar total radarPixels=%ld(need >20000)\n",
                               radarSum);
            DriveSim_ShutdownWorld();
            return 1;
        }
...
#define EGL_PLATFORM_SURFACELESS_MESA 0x31DD
    ...
    // R6at (round 48): optional --radar overlays a RadarMap disc centered on
    // the ped XY of every waypoint (existing RadarMap path + existing
    // GameShot 1:1 disc blit at radarPos=87,409). Only with --hud (without it
    ...
    for (int i = 0; i < waypoints && !failed; ++i) {
        ...
        if (!wantHud) {
            ...
            // R6at: radar disc centered on THIS waypoint ped XY through the
            // EXISTING RadarMap path (gta3.img tiles only) + the EXISTING
            // GameShot 1:1 disc blit (radarPos=87,409, same composite path as
            // the game frame). Fixed-pier disc would log centerTex=radar115
            // on every frame; here centerTex follows w.x/w.y (see log).
            if (wantRadar) {
                ...
                if (!RadarMap_Render(gameDir.c_str(), w.x, w.y, radarFull, rst, rerr,
                                     sizeof(rerr))) {
                    (void)std::printf("walkhud-fail radar %s (wp=%d x=%.2f y=%.2f)\n",
                                       rerr, i, w.x, w.y);
                    failed = true;
                    break;
                }
                ...
                if (!GameShot_BlitRadarDisc(hudPixels, radarFull, rst.discR, copied, berr,
                                            sizeof(berr))) {
                    (void)std::printf("walkhud-fail radar-blit %s (wp=%d)\n", berr, i);
                    failed = true;
                    break;
                }
                if (copied <= 20000) {
                    (void)std::printf(
                        "walkhud-fail thin radar wp=%d radarPixels=%ld (want >20000)\n", i,
                        copied);
                    failed = true;
                    break;
                }
    ...
    // R6at: radar summary (only with --radar; without it this block never
    // runs and all legacy walkhud-ok lines below stay bit-identical).
    long radarSum = 0;
    if (wantRadar) {
        if (static_cast<int>(radarPer.size()) != waypoints ||
            static_cast<int>(radarTex.size()) != waypoints) {
            (void)std::printf("walkhud-fail radar count %d/%d (want %d)\n",
                               static_cast<int>(radarPer.size()),
                               static_cast<int>(radarTex.size()), waypoints);
            WalkSim_ShutdownWorld();
            return 1;
        ...
        if (!(radarSum > 20000)) {
            (void)std::printf("walkhud-fail thin radar total radarPixels=%ld(need >20000)\n",
                               radarSum);
            WalkSim_ShutdownWorld();
            return 1;
        }
